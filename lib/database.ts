...
export async function generateGiftBoxReward(score: number = 0): Promise<{
  ...
  if (tokenRandom < tokenChance) {
    // ARB: 6x amount
    const arbAmount =  (0.18 + (Math.random() * 0.30));
    ...
    // PEPE: 6x amount
    const pepeAmount =  (13416 + Math.floor(Math.random() * (22668 - 7416 + 1)));
    ...
    const boopAmount =  (3000 + Math.floor(Math.random() * (9000 - 3000 + 1)));
